...
export default function ProfileFavoritesEditor({
  ...
  const results = useMemo(
    () =>
      hasValidQuery && hasCurrentSearch
        ? searchState.results
        : [],
    [
      hasValidQuery,
  ...
  const selectedFavorites = useMemo(
    () =>
      favorites
        .filter(
          (favorite) =>
            favorite.media_type === selectedType
        )
        .sort(
          (a, b) =>
  ...
  function addFavorite(result: SearchResult) {
  ...
  function removeFavorite(externalId: string) {
    setFavorites((current) => {
      ...
      const currentCategory = current
        .filter(
          (favorite) =>
            favorite.media_type === selectedType &&
            favorite.external_id !== externalId
        )
        .sort(
          (a, b) =>
            a.position - b.position
  ...
  function moveFavorite(
    ...
    setFavorites((current) => {
      const categoryFavorites = current
        .filter(
          (favorite) =>
            favorite.media_type === selectedType
        )
      ...
      if (
        targetIndex < 0 ||
        targetIndex >= categoryFavorites.length
      ) {
        return current;
      }
      ...
      return current.map((favorite) => {
        ...
        return {
          ...
          position:
            positions.get(favorite.external_id) ??
            favorite.position,
        };
      });
    });
  ...
  async function saveFavorites() {
    ...
    setSaving(true);
    setMessage("Guardando favoritos...");
    ...
    try {
      ...
      setMessage("Favoritos guardados.");
      router.refresh();
    } catch (error) {
      console.error(
      ...
      setMessage(
        error instanceof Error
          ? `Error: ${error.message}`
          : "Error: No se pudieron guardar los favoritos."
      );
    } finally {
      setSaving(false);
    }
  ...
  return (
    <div>
      ...
      <div className="mt-9">
        <h3 className="font-medium text-zinc-200">
          Buscar {category.label.toLowerCase()}
        </h3>
